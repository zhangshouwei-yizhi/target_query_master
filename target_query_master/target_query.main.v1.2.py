...
class ParserManager:
    ...
    # 类常量 - 通用默认配置
    DEFAULT_R_SOFTWARE_PATH = "/home/ec2-user/software/annoconda/bin/envs/r4.3/bin/Rscript"
    
    # R脚本映射关系
    R_SCRIPT_MAPPING = {
        'depmap': 'DepMap_database_apply.r',
        'gtex': 'GTEx_database_apply.visual.r', 
        'tcga': 'TCGA_database_apply.r',
        'met500': 'MET500_database_apply.r'
    }

    def __init__(self,r_software_path=None,):
        self.parsers = {
            'hpa': HPAParser,
        ...
        # 设置通用参数，支持灵活的默认值配置
        self.r_software_path = r_software_path or self.DEFAULT_R_SOFTWARE_PATH

    
    def run_visualization(self, data_source, json_file_path, output_dir=None):
        """执行对应的R可视化脚本
        
        Args:
            data_source: 数据源名称 ('depmap', 'gtex', 'tcga', 'met500')
            json_file_path: JSON数据文件路径
            output_dir: 输出目录，默认为self.output_dir
        ...
        try:
            r_script_name = self.R_SCRIPT_MAPPING.get(data_source.lower())
            if not r_script_name:
                print(f"⚠️ 未知的数据源: {data_source}，跳过可视化")
                return False
            script_directory = os.path.dirname(os.path.abspath(__file__))
            r_visual_dir = os.path.join(script_directory,"r_visual")
            r_script_path = os.path.join(r_visual_dir, r_script_name)
            ...
            # 构建并执行命令
            cmd = [self.r_software_path, r_script_path, json_file_path, output_dir]
            print(f"🔄 执行可视化命令: {' '.join(cmd)}")
            ...
            if result.returncode == 0:
                print(f"✅ {data_source.upper()} 可视化完成")
                if result.stdout:
                    print(f"   R脚本输出: {result.stdout.strip()}")
                return True
            else:
                print(f"❌ {data_source.upper()} 可视化失败")
                print(f"   错误信息: {result.stderr}")
                return False 
        except subprocess.TimeoutExpired:
            print(f"❌ {data_source.upper()} 可视化超时（5分钟）")
            return False
        except Exception as e:
            print(f"❌ {data_source.upper()} 可视化执行异常: {e}")
            return False
    ...
    def load_config_file(self, config_file_path):
        ...
        try:
            ...
            with open(config_file_path, 'r') as f:
                ...
                # 从配置文件更新通用参数
                self._update_common_params_from_config()
    ...
    def _update_common_params_from_config(self):
        """从配置文件更新通用参数"""
        if not self.config_data:
            return
            
        common_config = self.config_data.get('common_params', {})
        
        # 更新R软件路径
        if 'r_software_path' in common_config:
            self.r_software_path = common_config['r_software_path']
            print(f"📝 从配置更新R脚本路径: {self.r_software_path}")
    ...
    def validate_common_files(self, args):
        ...
        # 从命令行参数更新通用参数
        self._update_common_params_from_args(args)
        ...
        common_files_info['output_dir'] = self.output_dir
        
        # 添加通用参数到common_files
        common_files_info['r_script_path'] = self.r_script_path
        
        self.common_files = common_files_info
        return common_files_info

    def _update_common_params_from_args(self, args):
        """从命令行参数更新通用参数"""
        # 更新R脚本路径
        if hasattr(args, 'r_software_path') and args.r_software_path:
            self.r_software_path = args.r_software_path
...
def setup_argparse():
    ...
    def add_common_arguments(subparser):
        ...
        subparser.add_argument('--r_software_path', 
                             help=f'Rscript可执行文件路径（默认: {ParserManager.DEFAULT_R_SOFTWARE_PATH}）', 
                             dest='r_software_path')
        return subparser
...
def run_hpa_parser(args):
    ...
    try:
        ...
        hpa_odir = os.path.join(odir,"HPA")
        manager.ensure_output_dir(hpa_odir)
        print("✅ 通用文件验证通过")
        ...
        hpa_parser.save_to_file(f"{hpa_odir}/{gene}.{ensembl_id}.HPA_data.json")
        print("✅ HPA数据解析完成")
...
def run_uniprot_parser(args):
    ...
    try:
        ...
        gene_info = common_files['gene_info']
        odir = common_files['output_dir']
        uniprot_odir = os.path.join(odir,"UniProt")
        manager.ensure_output_dir(uniprot_odir)
        print("✅ 通用文件验证通过")
...
def run_gtex_parser(args):
    ...
    try:
        ...
        gene_info = common_files['gene_info']
        odir = common_files['output_dir']
        gtex_odir = os.path.join(odir,"GTEx")
        manager.ensure_output_dir(gtex_odir)       
        print("✅ 通用文件验证通过")
        ...
        json_output_path = f"{gtex_odir}/{gene}.{ensembl_id}.GTEx_data.json"
        gtex_parser.save_to_file(json_output_path) 
        print("✅ GTEx数据解析完成")
        print(f"   目标基因: {gene} Ensembl: {ensembl_id}")

        # 6. 执行GTEx可视化
        print("🎨 开始GTEx数据可视化...")
        visualization_success = manager.run_visualization('gtex', json_output_path, gtex_odir)
        if visualization_success:
            print(f"✅ GTEx数据解析和可视化全部完成")
        else:
            print(f"⚠️ GTEx数据解析完成，但可视化失败")         
...
def run_depmap_parser(args):
    ...
    try:
        ...
        gene_info = common_files['gene_info']
        odir = common_files['output_dir']
        depmap_odir = os.path.join(odir,"DepMap")
        manager.ensure_output_dir(depmap_odir)
        print("✅ 通用文件验证通过")
        ...
        json_output_path = f"{depmap_odir}/{gene}.{ensembl_id}.DepMap_data.json"      
        depmap_parser.save_to_file(json_output_path)       
        print(f"✅ DepMap数据解析完成")
        print(f"   目标基因: {gene} Ensembl: {ensembl_id}")

        # 6. 执行DepMap可视化
        print("🎨 开始DepMap数据可视化...")
        visualization_success = manager.run_visualization('depmap', json_output_path, depmap_odir)
        if visualization_success:
            print(f"✅ DepMap数据解析和可视化全部完成")
        else:
            print(f"⚠️ DepMap数据解析完成，但可视化失败")
    except (FileNotFoundError, ValueError, PermissionError) as e:
...
def run_tcga_parser(args):
    ...
    try:
        ...
        gene_info = common_files['gene_info']
        odir = common_files['output_dir']
        tcga_odir = os.path.join(odir,"TCGA")
        manager.ensure_output_dir(tcga_odir)
        print("✅ 通用文件验证通过")
        ...
        json_output_path = f"{tcga_odir}/{gene_info['gene_symbol']}.{gene_info['ensembl']}.TCGA_data.json"
        tcga_parser.save_to_file(json_output_path)
        ...
        # 6. 执行TCGA可视化
        print("🎨 开始TCGA数据可视化...")
        visualization_success = manager.run_visualization('tcga', json_output_path, tcga_odir)
        
        if visualization_success:
            print(f"✅ TCGA数据解析和可视化全部完成")
        else:
            print(f"⚠️ TCGA数据解析完成，但可视化失败")         
    except (FileNotFoundError, ValueError, PermissionError) as e:
        print(f"❌ TCGA解析错误: {e}")
        sys.exit(1)
...
def run_met500_parser(args):
    ...
    try:
        ...
        gene_info = common_files['gene_info']
        odir = common_files['output_dir']
        met500_odir = os.path.join(odir,"MET500")
        manager.ensure_output_dir(met500_odir)
        print("✅ 通用文件验证通过")
        ...
        json_output_path = f"{met500_odir}/{gene}.{ensembl_id}.MET500_data.json"
        met500_parser.save_to_file(json_output_path)  
        print("✅ MET500数据解析完成")
        print(f"   目标基因: {gene} Ensembl: {ensembl_id}")

        # 6. 执行MET500可视化
        print("🎨 开始MET500数据可视化...")
        visualization_success = manager.run_visualization('met500', json_output_path, met500_odir)
        
        if visualization_success:
            print(f"✅ MET500数据解析和可视化全部完成")
        else:
            print(f"⚠️ MET500数据解析完成，但可视化失败")
    except (FileNotFoundError, ValueError, PermissionError) as e:
